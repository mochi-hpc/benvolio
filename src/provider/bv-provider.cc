...
static char* cache_malloc(std::vector<char*> *cache_backup_memory, const uint64_t page_size) {
    if (cache_backup_memory->size()) {
        char* result = cache_backup_memory[0][0];
        cache_backup_memory->erase(cache_backup_memory->begin());
        return result;
    } else {
...
static void cache_free(std::vector<char*> *cache_backup_memory, char* mem_ptr) {
    cache_backup_memory->push_back(mem_ptr);
}
...
static void cache_remove_file(Cache_info *cache_info, std::string file) {
    ...
    for ( it2 = cache_file_table->begin(); it2 != cache_file_table->end(); ++it2 ) {
        ...
        cache_free(cache_backup_memory, it2->second->second);
        delete it2->second;
    }
...
static size_t cache_fetch_match(char* local_buf, Cache_file_info *cache_file_info, const off_t file_start, const uint64_t file_size) {
    std::lock_guard<tl::mutex> guard(*(cache_file_info->cache_mutex));
    uint64_t my_provider;
    ...
    int stripe_count, stripe_size;
    unsigned j;
    printf("entering obsolete function cache_match!!!!!!!!!!!");
    #if BENVOLIO_CACHE_STATISTICS == 1
...
*/
    ...
    bv_svc_provider(tl::engine *e, abt_io_instance_id abtio,
        ...
        : tl::provider<bv_svc_provider>(*e, provider_id), engine(e), gid(gid), pool(pool), abt_id(abtio), bufsize(b), xfersize(x) {
            ...
            printf("implementation version v3, hostname = %s, ssg_rank %d initialized with BENVOLIO_CACHE_MAX_N_BLOCKS = %d, BENVOLIO_CACHE_MIN_N_BLOCKS = %d, BENVOLIO_CACHE_MAX_BLOCK_SIZE = %d\n", hostname, ssg_rank, BENVOLIO_CACHE_MIN_N_BLOCKS, BENVOLIO_CACHE_MAX_N_BLOCKS, BENVOLIO_CACHE_MAX_BLOCK_SIZE);
